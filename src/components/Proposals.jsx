...
import { payoutBeneficiary } from '../LavishLair'
...
const Proposals = () => {
  ...
  const [blockAverageTime, setBlockAverageTime] = useState(null)
    ...
    window.web3.eth.getBlockNumber(function(err, nowBlockNumber) {
      ...
      window.web3.eth.getBlock(nowBlockNumber, function(err, nb) {
        ...
        window.web3.eth.getBlock(nowBlockNumber - 500, function(err, thenBlock) {
          ...
          setBlockAverageTime((nowBlock.timestamp - thenBlock.timestamp) / 500.0);
        });
      });
    });
  }
              ...
              <tbody>
                {proposals.map((proposal) => (
                  ...
                  >
                    ...
                    <td className="text-sm font-light px-6 py-4 whitespace-nowrap">
                      {Number(currentBlock.number) > Number(proposal.endBlock)
                        ? 'Expired'
                        : ( blockAverageTime ? daysRemaining(currentBlock, proposal.endBlock, blockAverageTime)
                        : 'Unknown Deadline')
                        }
                    </td>
                    <td
